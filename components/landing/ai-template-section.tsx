...
const EXAMPLES: Example[] = [
  ...
  {
    level: "Medio",
    request: "Tengo una verdulería, etiquetas para pesar fruta y verdura suelta — nombre del producto, precio por kilo y peso",
    reply: "Diseño para balanza: nombre grande, precio por kilo, peso y un código de barras.",
    widthMm: 60,
    heightMm: 40,
    elements: [
      { id: "1", type: "text", content: "{{nombre_producto}}", x: 30, y: 30, fontSize: 15, bold: true },
      { id: "2", type: "text", content: "Precio/Kg: ${{precio_kilo}}", x: 30, y: 110, fontSize: 9 },
      { id: "3", type: "text", content: "Peso: {{peso}} kg", x: 30, y: 170, fontSize: 9 },
      { id: "4", type: "barcode", content: "{{codigo_barras}}", x: 30, y: 260, fontSize: 8 },
    ],
  },
  {
    level: "Complejo",
...
function MiniPreview({ example }: { example: Example }) {
  ...
  return (
    <div className="flex justify-center py-4">
      <div className="relative border-2 border-dashed border-border bg-white shadow-sm" style={{ width, height }}>
        {example.elements.map((el) => {
          ...
          return (
            <span
              ...
              style={{ left, top, fontSize: Math.max(7, el.fontSize * SCALE / 3), fontWeight: el.bold ? "bold" : "normal" }}
            >
              {el.content}
            </span>
          )
