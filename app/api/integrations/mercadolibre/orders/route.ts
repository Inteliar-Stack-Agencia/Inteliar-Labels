...
import { fetchOrderRows, isMercadolibreConfigured, type ImportMode } from "@/lib/mercadolibre"
...
// POST /api/integrations/mercadolibre/orders
...
export async function POST(req: NextRequest) {
  ...
  const { allowed } = await checkRateLimit(supabaseAdmin, "ml-orders-sync", user.id, 10, 5 * 60)
  if (!allowed) {
