...
import { fetchCatalogRows, isTiendanubeConfigured } from "@/lib/tiendanube"
...
// POST /api/integrations/tiendanube/catalog
// Full published product catalog, via the authenticated store connection.
export async function POST() {
  ...
  const { allowed } = await checkRateLimit(supabaseAdmin, "tn-catalog-sync", user.id, 10, 5 * 60)
  if (!allowed) {
    return NextResponse.json({ error: "Demasiadas sincronizaciones seguidas. Esperá unos minutos." }, { status: 429 })
  }

  try {
    const { columns, rows } = await fetchCatalogRows(user.id)
    if (rows.length === 0) {
